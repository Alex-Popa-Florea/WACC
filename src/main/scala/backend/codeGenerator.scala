...
object codeGenerator {
    ...
    */
    ...
    def writeToFileWithDir(lines: List[Line], fileName: String): Unit = {
        val file = new File(fileName)
    ...
    */
    def generateNode(node: Node, symbolTable: SymbolTable, functionTable: FunctionTable, label: Scope,
                     dataMap: Map[Scope, Msg], textMap: Map[Scope, ListBuffer[Instruction]]): Unit = {
        ...
        node match {
            ...
            case Begin(func, stat) =>
                ...
                */
                var i = decrementStack(symbolTable.getSize())
                ...
                */
                i = incrementStack(symbolTable.getSize())
            ...
            case function: Function =>
                ...
                /*
                    Decrements stack based on function stack size.
                */
                var i = decrementStack(functionStackSize)
            ...
            case Read(lhs) => 
                ...
                lhs match {
                    case ident: Ident => generateLHSIdentAndArrayElem(ident, symbolTable)

                    case ArrayElem(ident, _) => generateLHSIdentAndArrayElem(ident, symbolTable)

                    case Fst(expr) => generateLHSPairs(expr, true)
                        
                    case Snd(expr) => generateLHSPairs(expr, false)
                }
            ...
            case Return(expr) => 
                generateExpr(expr, symbolTable, functionTable, label, 4, dataMap, textMap)
                textMap(label).addOne(MOV(None, false, R(0), R(4)))
                var i = incrementStack(functionStackSize)
                textMap(label).addOne(POP(List(PC())))

            case Exit(expr) => 
                generateExpr(expr, symbolTable, functionTable, label, 4, dataMap, textMap)
                textMap(label).addAll(ListBuffer(MOV(None, false, R(0), R(4)), BL(None, "exit")))

            case print: PrintTrait =>
                generateExpr(print.expr, symbolTable, functionTable, label, 4, dataMap, textMap)
                textMap( ).addOne(MOV(None, false, R(0), R(4)))
                textMap(label).addOne(print.expr match {
                        ...
                        ident.symbolTable.get.find(ident) match {
                            case Some(typeCheck) => typeCheck match {
                                ...
                                case BoolCheck(nested) =>
                                    if
                                            Method that generates assembly level code for printing booleans. (nested == 0)    /*
                                    */{
                            ...
                            case None => BL(None, "p_print_reference") // TODO :think 
                        }
                    ...
                    case ArrayElem(id, exprs) => 
                        ...
                        id.symbolTable.get.find(id) match {
                            case Some(value) => value match {
                                ...
                                case BoolCheck(nested) =>
                                    if
                                            Method that generates assembly level code for printing booleans. (nested == siz   /*
                                    */e) {
                                        generatePrintBool(dataMap, textMap)
                                        BL(None, "p_print_bool")
                                    } else {
                    ...
                    ca
                        Method that generates assembly level code for printing booleans.se BoolLiter(bo   /*
                    */ol) => 
                        generatePrintBool(dataMap, textMap)
                        BL(None, "p_print_bool")
                    ...
                    ca
                        Method that generates assembly level code for printing booleans.se Not(expr1) =   /*
                    */>
                        generatePrintBool(dataMap, textMap)
                        BL(None, "p_print_bool")
                    ...
                    ca
                        Method that generates assembly level code for printing booleans.se _: BinOpBool   /*
                    */ => 
                        generatePrintBool(dataMap, textMap)
                        BL(None, "p_print_bool")
                    
                    ca
                        Method that generates assembly level code for printing booleans.se _: BinOpEqs    /*
                    */=> 
                        generatePrintBool(dataMap, textMap)
                        BL(None, "p_print_bool")
                    
                    ca
                        Method that generates assembly level code for printing booleans.se _: BinOpComp   /*
                    */ => 
                        generatePrintBool(dataMap, textMap)
                        BL(None, "p_print_bool")
                })
            ...
            case ifStatement: If => 
                ...
                /*
                    Decrements stack based on number of assignments in true branch of if statement.
                */
                var i = decrementStack(ifStatement.trueSemanticTable.getOrElse(symbolTable).getSize())
                functionStackSize += i
                ...
                /*
                    Increments stack back to its original position.
                */
                i = incrementStack(ifStatement.trueSemanticTable.getOrElse(symbolTable).getSize())
                functionStackSize -= i
                ...
                /*
                    Decrements stack based on number of assignments in false branch of if statement. 
                */
                i = decrementStack(ifStatement.falseSemanticTable.getOrElse(symbolTable).getSize())
                functionStackSize += i
                ...
                /*
                    Increments stack back to its original position.
                */
                i = incrementStack(ifStatement.falseSemanticTable.getOrElse(symbolTable).getSize())
                functionStackSize -= i          
            ...
            case whileStatement: While => 
                ...
                /*
                    Decrements stack based on number of assignments in while statement. 
                */
                var i = decrementStack(whileStatement.semanticTable.getOrElse(symbolTable).getSize())
                functionStackSize += i
                ...
                /*
                    Increments stack back to its original position.
                */
                i = incrementStack(whileStatement.semanticTable.getOrElse(symbolTable).getSize())
                functionStackSize -= i   
            ...
            case nestedBegin: NestedBegin =>
                ...
                /*
                    Decrements stack based on number of assignments in nested begin. 
                */
                var i = decrementStack(nestedBegin.semanticTable.getOrElse(symbolTable).getSize())
                functionStackSize += i
                ...
                /*
                    Increments stack back to its original position.
                */
                i = incrementStack(nestedBegin.semanticTable.getOrElse(symbolTable).getSize())
                functionStackSize -= i
                    ...
                    case ident: Ident => 
                        ...
                        */
                        (ident.symbolTable.get.find(ident): @unchecked) match {
                            /*
                                Uses @unchecked to avoid warnings of non exhaustive case matching.
                                Type is always PairCheck.
                            */
                            case Some(typeCheck) => (typeCheck: @unchecked) match {
                                case PairCheck(type1, _, _) => 
                                    if (getBytesFromType(type1) == 4) {
                                        textMap(label).addOne(LDR(None, R(register), ZeroOffset(R(register))))
                                    } else {
            ...
            case Call(id, args) => 
                args.reverse.map(arg => {
                    ...
                    if (getBytes(arg, symbolTable) == 1) {
                        textMap(label).addOne(STRB(None, R(4), RegisterOffset(SP(), Immed(-1))))
                        stackOffset += 1
                    } else {
                        textMap(label).addOne(STR(None, R(4), RegisterOffset(SP(), Immed(-4))))
                        stackOffset += 4
                    }
                })
                stackOffset = 0
            ...
            case ident: Ident => 
                ...
                */
                (ident.symbolTable.get.find(ident): @unchecked) match {
                    /*
                        Uses @unchecked to avoid warnings of non exhaustive case matching.
                        Type is always either BoolCheck or CharCheck.
                    */
                    case Some(typeCheck) => (typeCheck: @unchecked) match {
                        case BoolCheck(nested) =>
                            if (nested == 0) {
    ...
    */
    def decrementStack(size: Int): Int = {
        while (size > 0) {
    ...
    */
    def incrementStack(size: Int): Int = {
        while (i > 0) {
            if (i > MAX_NUM_BYTES) {
    ...
    */
    def generateExpr(expr: Expr, symbolTable: SymbolTable, functionTable: FunctionTable, label: Scope, register: Int, dataMap: Map[Scope, Msg], textMap: Map[Scope, ListBuffer[Instruction]]): Unit = {
        ...
        expr match {
            ...
            case IntLiter(x) => textMap(label).addOne(LDR(None, R(register), Immed(x)))
            ...
            case CharLiter(char) => textMap(label).addOne(MOV(None, false, R(register), Character(char)))
            ...
            case PairLiter() => textMap(label).addOne(LDR(None, R(register), Immed(0)))
            ...
            case Ord(expr1) => generateExpr(expr1, symbolTable, functionTable, label, register, dataMap, textMap)
            
            case Chr(expr1) => generateExpr(expr1, symbolTable, functionTable, label, register, dataMap, textMap)
    ...
    */
    ...
    /*
        Method that generates assembly level code for lhs expressions of type Ident or ArrayElem.
    */
    def generateReadIdentAndArrayElem(id: Ident, symbolTable: SymbolTable): Unit = {
        /*
            Uses @unchecked to avoid warnings of non exhaustive case matching.
            Ident always has a type in the symbol table.
        */
        (id.symbolTable.get.find(id): @unchecked) match {
            /*
                Uses @unchecked to avoid warnings of non exhaustive case matching.
                Type is always either IntCheck or CharCheck.
            */
            case Some(typeCheck) => (typeCheck: @unchecked) match {
                case IntCheck(_) => 
    ...
    /*
        Method that generates assembly level code for fst and snd of LHS expression pairs. 
    */
    def generateReadPairs(expr: Expr, fst: bool): Unit = {
        expr match {
            case ident: Ident => 
                ident.symbolTable.get.find(ident) match {
                    case Some(typeCheck) => typeCheck match {
                        /*
                            ...
                            Type is always either IntCheck or CharCheck.
                        */
                        case PairCheck(type1, _, _) if fst => (type1: @unchecked) match {
                            case IntCheck(_) => 
                                generateReadInt(dataMap, textMap)
                                textMap(label).addOne(BL(None, "p_read_int"))    
                        ...
                        /*
                            ...
                            Type is always either IntCheck or CharCheck.
                        */
                        case PairCheck(_, type2, _) => (type2: @unchecked) match {
                            case IntCheck(_) => 
                                generateReadInt(dataMap, textMap)
                                textMap(label).addOne(BL(None, "p_read_int"))    
